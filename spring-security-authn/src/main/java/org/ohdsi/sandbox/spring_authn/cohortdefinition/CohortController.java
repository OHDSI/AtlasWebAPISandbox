...
/**
 * Demo controller for cohort definition operations with entity-level authorization.
 * 
 * This controller demonstrates the authorization model without actual CRUD operations.
...
public class CohortController {
     ...
     */
    @GetMapping
    public Map<String, String> listCohorts() {
        String username = authorizationService.getCurrentLogin();
        log.info("listCohorts called by: {}", username);
        return Map.of(
            "message", "Public cohort list accessible to all users",
            "user", username
        );
    }
     ...
     * Authorization: isOwner OR isPermitted('write:cohort') OR hasEntityAccess(WRITE)
